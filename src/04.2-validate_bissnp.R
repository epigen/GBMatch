...
#  simpleCache(recreate=TRUE,cachename,{dt=condense_vcf(sample=annotation_val$N_number_seq[i],patient=annotation_val$patID[i],vcf_file=annotation_val$vcfs[i],vcf_bg_file=annotation_val$vcf_bg[i],covThres=covThres);return(dt)},cacheSubDir=paste0("wgs_var_bissnp_cov",covThres))  
cachename=paste0("rrbs_",annotation_val$N_number_seq[i])
message(cachename)
simpleCache(recreate=TRUE,cachename,{dt=condense_vcf(sample=annotation_val$N_number_seq[i],patient=annotation_val$patID[i],vcf_file=annotation_val$vcfs_RRBS[i],vcf_bg_file=annotation_val$vcf_bg_RRBS[i],covThres=covThres);return(dt)},cacheSubDir=paste0("rrbs_var_bissnp_cov",covThres))
  ...
  rrbs_var=rbindlist(list(rrbs_var,var))
}
...
ggplot(var_samp_compare,aes(x=wgs_var_count_norm,y=rrbs_var_count_norm))+geom_point()+geom_smooth(method="lm")+geom_abline(slope=1,intercept=c(0,0),size=2,col="grey",alpha=0.7)+facet_wrap(~count_type,scale="free")
