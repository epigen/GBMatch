...
for (covariate in covariates){
  print(covariate)
  data=annotation_wide[category%in%sel_categories]
#  if (sel_categories=="GBmatch_val"){data=data[plate_dna_abbrev.1!="EA003_plate2"]}
...
write.table(uni_models,paste0(paste0(sel_categories,collapse="_"),"_",mode,"_uni_models.tsv"),sep="\t",quote=FALSE,row.names=FALSE)

#multivariate model with significant covariates
multi_formula=as.formula(paste0("Surv(time = ",mode,", event = VitalStatus) ~ ",paste0("`",uni_models[pvalue<0.05]$covariate,"`",collapse="+")))
multi_cox=summary(coxph(multi_formula , data=annotation_wide[category%in%sel_categories]))

capture.output(multi_cox,file=paste0(paste0(sel_categories,collapse="_"),"_",mode,"_multi_model_signif.txt"))
