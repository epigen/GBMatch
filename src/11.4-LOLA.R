...
for (lola_cohort in cohorts){
  ...
  if(!file.exists(dirout(out, paste0(paste0(lola_cohort,collapse="_"),"_LOLA_FULL.tsv")))){
    ...
    for(xnam in names(userSets)){
      tryCatch({
        ...
        res.x$userSet_N <- rep(length(userSets[[xnam]]$query), nrow(res.x))
  ...
  print(ggplot(LOLA_res[BY<0.001],aes(x=target,y=-log10(BY),size=logOddsRatio,fill=cellType_corr))+geom_point(position=position_jitter(height=0,width=0.1),shape=21,alpha=0.6,stroke=0.6)+geom_hline(yintercept=-log10(0.001),lty=20,col="grey")+theme(axis.text.x=element_text(angle = 90, hjust=1,vjust = 0.5))+facet_wrap(~userSet+userSet_N,ncol=1)+scale_fill_manual(values=c("Astrocyte"="#a6cee3","ESC"="#b2df8a"))+scale_color_manual(values=c("FALSE"="grey","TRUE"="black")))
  dev.off()
  write.table(LOLA_res[BY<0.001,list(target,BY,logOddsRatio,cellType_corr,userSet)],dirout(out, paste0(paste0(lola_cohort,collapse="_"),"_LOLA_res_subgroup_BY0.001_data.csv")),sep=";",row.names=FALSE,quote=FALSE)
